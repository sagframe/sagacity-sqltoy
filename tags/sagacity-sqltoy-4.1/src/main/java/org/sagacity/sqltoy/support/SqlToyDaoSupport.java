...
public class SqlToyDaoSupport {
	 ...
	 * @throws Exception
	 */
	protected Long executeSql(final String sqlOrNamedSql) throws Exception {
	...
	protected List findByCriteria(final Criteria criteria) throws Exception {
		...
		return findByQuery(new QueryExecutor(queryStr.toString(), null, criteria.getParamValues().toArray())
				.resultType(criteria.getEntity().getClass())).getRows();
	}
	...
	protected PaginationModel findPageByCriteria(PaginationModel pageModel, Criteria criteria) throws Exception {
		...
		// 增加一个空格增强容错
		queryStr.append(" ").append(criteria.wrapSqlFragments(entityMeta));
		return findPageByQuery(pageModel,
				new QueryExecutor(queryStr.toString(), null, criteria.getParamValues().toArray())
						.resultType(criteria.getEntity().getClass())).getPageResult();
	}
