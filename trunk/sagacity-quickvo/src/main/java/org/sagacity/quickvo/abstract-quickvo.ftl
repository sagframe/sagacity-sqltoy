...
<#if (column.colRemark?exists && column.colRemark!='')>
	...
	@BusinessId(generator="${businessId.generator}"<#if (businessId.signature?exists)>,signature="${businessId.signature}"</#if><#if (businessId.relatedColumns?exists)>,relatedColumns={"${businessId.relatedColumns}"}</#if><#if (businessId.length?exists)>,length=${businessId.length}</#if>)
	</#if>
