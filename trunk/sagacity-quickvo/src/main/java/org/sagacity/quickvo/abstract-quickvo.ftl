...
<#if (column.colRemark?exists && column.colRemark!='')>
	...
	@BusinessId(generator="${businessId.generator}"<#if (businessId.signature?exists)>,signature="${businessId.signature}"</#if><#if (businessId.relatedColumn?exists)>,relatedColumn="${businessId.relatedColumn}"</#if><#if (businessId.length?exists)>,length=${businessId.length}</#if>)
	</#if>
