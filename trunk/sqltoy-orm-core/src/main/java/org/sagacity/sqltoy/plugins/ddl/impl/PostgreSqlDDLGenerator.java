...
public class PostgreSqlDDLGenerator implements DialectDDLGenerator {
	...
	public String createTableSql(TableMeta tableMeta, String schema, String upperOrLower, int dbType) {
		...
		for (ColumnMeta colMeta : tableMeta.getColumns()) {
			...
			// 类型:openGauss/Kingbase等不支持IDENTITY的库用BIGSERIAL替代自增
			if (colMeta.isAutoIncrement() && dbType != DBType.POSTGRESQL && dbType != DBType.POSTGRESQL14) {
				tableSql.append(" BIGSERIAL");
			} else {
			...
			if (colMeta.getGeneratedType() > 0 && StringUtil.isNotBlank(colMeta.getDefaultValue())) {
				tableSql.append(" GENERATED ALWAYS AS (").append(colMeta.getDefaultValue()).append(") STORED ");
			}
			// 是否为null
			if (!colMeta.isNullable()) {
				tableSql.append(" NOT NULL");
			}
			// 自增:标准PG用IDENTITY;非标准PG(openGauss等)已改为BIGSERIAL,跳过DEFAULT(BIGSERIAL自带隐式sequence默认值)
			if (colMeta.isAutoIncrement()) {
				if (dbType == DBType.POSTGRESQL || dbType == DBType.POSTGRESQL14) {
					tableSql.append(" GENERATED ALWAYS AS IDENTITY ");
				}
				// BIGSERIAL列跳过DEFAULT(数据库已隐式设nextval,重复DEFAULT会报multiple defaults)
			} else if (colMeta.getGeneratedType() == 0 && StringUtil.isNotBlank(colMeta.getDefaultValue())) {
				// 启发式:default以裸标识符开头后跟运算符,是生成列表达式而非字面量
				// (openGauss/vastbase等不暴露生成标记,COLUMN_DEF存的是表达式原文,DEFAULT形式PG拒绝)
				if (!colMeta.getDefaultValue().startsWith("'")
						&& colMeta.getDefaultValue().matches("[a-zA-Z_][a-zA-Z0-9_]*\\s*[+\\-*/].+")) {
					tableSql.append(" GENERATED ALWAYS AS (").append(colMeta.getDefaultValue()).append(") STORED");
				} else {
