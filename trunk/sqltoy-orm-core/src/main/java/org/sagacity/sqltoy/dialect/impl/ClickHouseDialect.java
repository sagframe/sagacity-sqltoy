...
public class ClickHouseDialect implements Dialect {
	...
	public boolean isUnique(SqlToyContext sqlToyContext, Serializable entity, String[] paramsNamed, Connection conn,
			DBProfile profile, final String tableName, final Integer queryTimeout) {
	...
	public QueryResult findBySql(SqlToyContext sqlToyContext, SqlToyConfig sqlToyConfig, String sql,
			...
			Connection conn, final LockMode lockMode, DBProfile profile, int fetchSize, int maxRows) throws Exception {
		// clickhouse目前不支持锁查询
		if (null != lockMode) {
	...
	public Object save(SqlToyContext sqlToyContext, Serializable entity, Connection conn, DBProfile profile,
		...
		String insertSql = DialectExtUtils.generateInsertSql(sqlToyContext.getUnifyFieldsHandler(), profile, entityMeta,
				pkStrategy, "NEXTVAL FOR " + entityMeta.getSequence(),
				ClickHouseDialectUtils.allowAssignPKValue(pkStrategy), tableName);
		return ClickHouseDialectUtils.save(sqlToyContext, entityMeta, pkStrategy, insertSql, entity, conn, profile);
	}
	...
	public Long saveAll(SqlToyContext sqlToyContext, List<?> entities, int batchSize,
		...
		// clickhouse 不支持sequence，支持identity自增模式
		String insertSql = DialectExtUtils.generateInsertSql(sqlToyContext.getUnifyFieldsHandler(), profile, entityMeta,
				entityMeta.getIdStrategy(), "NEXTVAL FOR " + entityMeta.getSequence(),
	...
	public Long update(SqlToyContext sqlToyContext, Serializable entity, String[] forceUpdateFields, boolean cascade,
		...
		return ClickHouseDialectUtils.update(sqlToyContext, entity, forceUpdateFields, conn, profile, tableName);
	}
	...
	public Long updateAll(SqlToyContext sqlToyContext, List<?> entities, int batchSize, final String[] uniqueFields,
		...
		return ClickHouseDialectUtils.updateAll(sqlToyContext, entities, batchSize, forceUpdateFields,
				reflectPropsHandler, conn, profile, autoCommit, tableName, false);
	}
	...
	public Long delete(SqlToyContext sqlToyContext, Serializable entity, Connection conn, DBProfile profile,
			String tableName) throws Exception {
		return ClickHouseDialectUtils.delete(sqlToyContext, entity, conn, profile, tableName);
	}
	...
	public Long deleteAll(SqlToyContext sqlToyContext, List<?> entities, int batchSize, Connection conn,
		...
		return DefaultDialectUtils.deleteAll(sqlToyContext, entities, batchSize, conn, profile, autoCommit, tableName);
	}
	...
	public StoreResult executeStore(SqlToyContext sqlToyContext, SqlToyConfig sqlToyConfig, String sql,
			...
			DBProfile profile, final int fetchSize, final Integer timeout) throws Exception {
		// 不支持
		throw new UnsupportedOperationException(SqlToyConstants.UN_SUPPORT_MESSAGE);
	}
	...
	public List<ColumnMeta> getTableColumns(String catalog, String schema, String tableName, Connection conn,
		...
		return ClickHouseDialectUtils.getTableColumns(catalog, schema, tableName, conn, profile);
	}
