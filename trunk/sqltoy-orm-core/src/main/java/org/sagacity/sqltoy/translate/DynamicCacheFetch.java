 ...
 *           <li> 1、配置 spring.sqltoy.dynamicCacheFetch=xxxx.xxx.DynamicCacheFetchImpl</li>
 *           <li> 2、在sqltoy-translates.xml 缓存定义配置中增加:sid、properties 可选，供开发者提供特定标记用
...
public interface DynamicCacheFetch {
	...
	// <local-translate cache="" sid="" properties="" keep-alive="3600" dynamic-cache="true" dynamic-cache-maxSize="100000"/>
	//第二步: 定义DynamicCacheFetch的实现类，如果用到spring获取bean，initialize(AppContext appContext)中传递了appContext.getBean(xxx)
	//第三步: 在项目配置文件中，定义spring.sqltoy.dynamicCacheFetch=xxxx.DynamicCacheFetchImpl 实现类(也可以直接是一个bean的名称）
	...
	//    public HashMap<String, FieldTranslateCacheHolder> getTranslates(HashMap<String, FieldTranslate> translates) 194行
	//3、参见：SqlToyContext.initialize()方法中默认实列化了467行:dynamicFecthCacheManager = new FIFODynamicFetchCacheManager();
	//4、参见：org.sagacity.sqltoy.utils.TranslateUtils
	
	/**
	 * @TODO 取单个值的缓存数据
	 * @param cacheName
	 * @param cacheType  类似数据字典的分组(使用缓存的地方传递<translate cache="skuDict" cache-type="clothes">)
	 *                   根据情况使用(非分组场景则为null)
	 ...
	 * @return
	 */
	public List<Object[]> getCache(String cacheName, String cacheType, String sid, String[] properties, String[] keys);
}