 ...
 * @author zhongxuchen
 * @version v1.0,Date:2018-01-17
 * @modify Date:2019-09-15 增加了forceUpdateFields方法
 * @modify Date:2021-10-11 增加了dataAuthFilters做统一参数传递和统一数据权限防越权过滤
 * @modify Date:2023-03-09 完善了SqlInterceptors，增加authTenants方法获取当前用户的授权租户id数组
 * @modify Date:2023-05-13 增加了createSqlTimeFields、updateSqlTimeFields
 *         解决增加和修改记录时，创建时间和修改时间采用数据库时间}
 * @modify Date:2025-06-05 增加getUserTenantId()方法获取当前用户所在租户id
 */
public interface IUnifyFieldsHandler {
	...
	/**
	 * 设置创建记录时需要赋值的字段和对应的值(弹性模式:即优先以传递的值优先，为null再填充)
	 * 
	 * @return
	 */
	 ...
	 * </p>
	 * 
	 * 数据权限过滤，你可以灵活结合AOP、ThreadLocal技术实现不同场景注入不同的数据权限值
	 * 
	 * @return
	 */
	...
	/**
	 * 获取授权租户信息，传递表名和操作类型目的为程序可以控制返回:所在租户和授权租户 提供部分决策依据
