...
public class SqlToyDaoChecker {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("----------------------------------------");
			methodGroups.entrySet().stream().sorted(Map.Entry.<String, List<Method>>comparingByKey().reversed())
					.forEach(entry -> {
			...
			checkImplementation("DefaultSqlToyDaoImpl", "org.sagacity.sqltoy.dao.impl.DefaultSqlToyDaoImpl",
					interfaceMethods);

			// 检查 Spring 模块的 SqlToyDaoImpl
			checkImplementation("SqlToyDaoImpl(Spring)", "org.sagacity.sqltoy.dao.impl.SqlToyDaoImpl",
					interfaceMethods);

			// 检查 Spring Starter 模块的 SqlToyDaoImpl
			checkImplementation("SqlToyDaoImpl(Spring Starter)", "org.sagacity.sqltoy.dao.impl.SqlToyDaoImpl",
					interfaceMethods);

			// 检查 Solon 模块的 SqlToyDaoImpl
			checkImplementation("SqlToyDaoImpl(Solon)", "org.sagacity.sqltoy.solon.dao.impl.SqlToyDaoImpl",
					interfaceMethods);
	...
	private static String getMethodPrefix(String methodName) {
		...
		if (methodName.startsWith("findPage"))
			return "findPage*";
		if (methodName.startsWith("findStream"))
			return "findStream*";
		...
		if (methodName.equals("count"))
			return "count";
		if (methodName.equals("elastic"))
			return "elastic";
		if (methodName.equals("mongo"))
			return "mongo";
		...
		if (methodName.equals("load"))
			return "load";
		if (methodName.equals("unique"))
			return "unique";
		if (methodName.equals("treeTable"))
			return "treeTable";
		if (methodName.equals("batch"))
			return "batch";
		if (methodName.equals("tableApi"))
			return "tableApi";
		if (methodName.equals("flush"))
			return "flush";
		if (methodName.equals("isUnique"))
			return "isUnique";
		if (methodName.equals("existCache"))
			return "existCache";
		if (methodName.equals("wrapTreeTableRoute"))
			return "wrapTreeTableRoute";
		return "other";
	}
}
