...
public class SqlUtilsExtBatchCountTest {
	...
	// update 2026-9-15 适配batchUpdateForPOJO末参dbType→DBProfile重构:profile为必传
	// (setParamValue经profile.getDbType()取库类型,null即NPE);代理连接metadata返回null
	// 不可走getDBProfile,按测试惯例构造UNDEFINE最小档案(与旧null dbType语义等价)
	...
	private static Long execute(int[] batchCounts) throws Exception {
		return SqlUtil.batchUpdateForPOJO(null, "insert into t values(?,?)", threeRows(), null, 100, null,
				connectionReturning(batchCounts), undefProfile());
	}
