...
public class SqlWithAnalysis implements Serializable {
	...
	private final Pattern withPattern = Pattern.compile(
			"(?i)\\s*with\\s+[a-z|0-9|\\_]+\\s*(\\([a-z|0-9|\\_|\\s|\\,]+\\))?\\s+as\\s*(materialized)?\\s*\\(");

	// with 下面多个as
	private final Pattern otherWithPattern = Pattern.compile(
			"(?i)\\s*\\,\\s*[a-z|0-9|\\_]+\\s*(\\([a-z|0-9|\\_|\\s|\\,]+\\))?\\s+as\\s*[materialized]?\\s*\\(");
	...
	private void parse() {
		...
		if (withAsMatcher.find()) {
			...
			asIndex = withAsMatcher.group().toLowerCase().indexOf(" as");
			// 剔除with
	...
	public static void main(String[] args) {
		// String sql = "with t1 as materialized (select * from table ) select * from
		// t1";
