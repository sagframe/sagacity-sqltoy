# QQ交流群:531812227
# 使用单位或项目: 宁波农行  山东农信  成都银行   中国建行上海开发中心  合肥农信  浦发电销  拉卡拉   中化壹化网  中化化销

# 致谢:
  sqltoy的发展感谢拉卡拉支付有限公司的支持，将sqltoy作为公司orm的主推框架。
  
  并于2018年7月收到拉卡拉公司的一笔6千元的创新资助。
  - 1、在拉卡拉sqltoy经受了日均1300万笔交易流水的考验
  - 2、通过拉卡拉的业务全场景验证:分库分表；缓存翻译的优势的展现；快速分页、分页优化在每一点性能都需要考虑的场景下价值体现；
       elasticsearch十亿级别的数据毫秒级查询；mongodb在用户画像方向的使用(用户信息标签化)
   
# 1. 前言
## 1.1 sqltoy-orm是什么

  sqltoy-orm是一个基于java实现的关系型数据库ORM框架，支持常规的数据库如oracle、mysql(以及mysql的衍生产品mariadb)、sqlserver、postgresql、db2、sybase ase、sybase iq等。
  sqltoy-orm包含3个部分：
  - 1、sagacity-sqltoy：orm核心部分。
  - 2、sagacity-quickvo:辅助工具部分，帮助开发者从数据库产生POJO。
  - 3、sagacity-sqltoy-starter:springboot集成的starter。
  
  sqltoy-orm是hibernate和mybatis的功能合集，具有hibernate等同的对象操作便捷性也有比mybatis更加直观强大的自定义sql编写、查询、处理功能。
  
  sqltoy-orm试图让开发者通过本框架的应用，实现自我能力的增强，掌握sqltoy让你之前很多复杂的技术问题变成了一种自然和平淡。

  一个项目数据库交互并不是唯一工作，愿sqltoy可以帮助您实现精力的聚焦，更多的精力可以关注其他未知或不可控的领域。
