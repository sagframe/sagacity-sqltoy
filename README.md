...
* 因为每个公司和团队的技术、项目情况不一样，会导致偏重不一，所以sqltoy在有些方面被检验的不够充分！比如之前的级联操作bug和存储过程查询bug，确实是互联网项目已经不提倡级联和存储过程！
* 但可以肯定的是4.13.13.1 版本已经将这些问题做了很好的清理，所以坑没有那么容易被踩到！
* 同时发现bug并不可怕，bug会在一天内解决!而且个人坚信现在如果能发现bug绝对是非常偏的场景，可以临时用其他方式先行处理!不会影响项目大局!
