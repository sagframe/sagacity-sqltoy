...
# 最新版本号: 4.13.10.1 发版日期: 2020-07-31
* 修复级联操作时因外键跟关联主键字段名称不一致导致的bug,建议升级!

# 即将到来的(预计8月中旬发版,目前在测试中)
*  将在4.13.11 版本中平衡xml和代码中写sql,全面迎接文本块的到来，可以将10行内的中小篇幅的sql写在代码中。
*  在4.13.11 版本中将全面完善mysql、postgres数据库update级联更新情况下子表saveOrUpdateAll的瑕疵(这个瑕疵是数据库缺陷导致,采用merge 语法的oracle、db2、mssql则不存在此问题)
