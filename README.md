...
* 2007~2008年，因项目的特殊性，灵感式发现了比mybatis强无数倍的动态sql写法，并作为hibernate jpa 查询方面的补充，收到了极为震撼的开发体验。
  可以看写于2009年的一篇博文: https://blog.csdn.net/iteye_2252/article/details/81683940
...
* 2014~2017年, 对sqltoy进行了大幅重构，实现了底层结构的合理化，增加了对不同方言的测试验证，并在拉卡拉CRM和日均千万级累计达几十亿级别的数据平台上得到了充分体现。
* 2018~至今,  在ERP复杂场景下得到了充分锤炼，得到了开发者高度的认可，大家建议对外开源让大家一起分享sqltoy的好的特性同时也减少大家使用的风险(一个封闭的底层框架会产生很多决策顾虑)。

## 1.3 sqltoy 是否造重复的轮子
* 我和很多人一样一样都希望用一个现成的，可以腾出大把精力用于其他领域!但非常不幸让我发现了动态sql的最合适的写法，无法再接受mybatis那种有辱代码的写法!
* sqltoy 是项目实践的总结，是亲身经历各类项目面对各种问题的应对策略的归纳，从小项目到复杂的erp，从单一数据库普通报表项目再到拉卡拉几十亿规模数据覆盖oracle rac、mongodb、elasticsearch、spark on hive 多种技术融合应用的数据平台，sqltoy就是构建这些项目的基础!
* sqltoy 主要是在常规功能之上额外提供：缓存翻译、分页优化、树形表处理、取随机记录、取top记录、更高效更符合业务场景的update操作、一次性更新并查询的updateFetch、分库分表、基于算法的行列旋转、分组汇总、同比环比等对项目过程有较大帮助的功能，让开发者可以较容易的应对复杂场景。
* 当前开源里面有不少ORM框架,并不想让大家造成选择困惑,996 真的不容易，换框架绝对是很忌讳的事情，请根据自己的实际情况来选择，再美好的技术放在996的大背景下都不好再说什么！
