...
# QQ交流群:531812227 
## 加群前小技能
* 辩证的理解项目的多样性,可以纯代码完成数据库交互，但也有需要写一些复杂sql的场景，而不是极端的强调其中一端
* sqltoy跟其他orm性能对比如何? 需要理解对系统性能影响的关键是IO,其中很大一部分是数据库交互,而数据库性能的关键往往取决于查询效率!sqltoy和其他ORM在自身代码上的性能差异应该不是系统性能的核心
* 初步了解sqltoy缓存翻译、快速分页、分页优化、分库分表等基本特性
* 问题会随着成长变得越来越复杂越多样,所以sqltoy不是jooq、不是mybatis、不是hibernate，而是一个你需要的时候可以帮到你，不需要时又不影响你的技术体系!
* 判断sqltoy对你是否有帮助,有可能sqltoy处于你的过往阶段,则当做一个小插曲小回忆飘过!
