...
# 能不能用于生产?有坑吗?
* sqltoy 其实一路经历金融类项目发展起来的,包含农行、建行、浦发、农信以及拉卡拉支付集团等，从几十万数据规模到几十亿数据规模一步步发展起来,神枪手是靠子弹喂出来的，sqltoy这么多独特的特性产生显然是经历了无数项目锤炼而来的!
* 因为每个公司和团队的技术、项目情况不一样，会导致偏重不一，所以sqltoy在有些方面被检验的不够充分！比如之前的级联操作和存储过程查询bug，确实是互联网项目已经不提倡级联和存储过程！
* 但可以肯定的是截止到4.13.13.1版本，经过不同用户不同数据库和各种场景的检验，很多边缘场景问题都已经得到了清理，坑没有那么容易被踩到！
* 搞技术选型最怕出问题背锅(当初就是你放弃所有大厂在用的mybatis非要选择这个名不见经传的sqltoy的)，为什么sqltoy前一段时间发版频繁?因为绝对不能让大家为选择sqltoy而成为背锅侠!
