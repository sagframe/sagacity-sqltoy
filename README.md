...
# 即将到来的4.13.11 (测试中，预计8月中下旬发版)
*  平衡xml和代码中写sql,全面迎接文本块的到来，便于中小篇幅的sql写在代码中(大篇幅的建议跟代码分离即使是文本块)。
*  将全面完善mysql、postgres数据库update级联更新情况下子表saveOrUpdateAll的瑕疵(这个瑕疵是数据库缺陷导致,采用merge 语法的oracle、db2、mssql则不存在此问题)
*  优化isUnique方法，变成单次查询
